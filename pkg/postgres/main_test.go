	...
	testutil.New(m,
		...
		testutil.OptBefore(
			func(ctx context.Context) error {
				return postgres.Migrations().Apply(ctx, testutil.DefaultDB())
			},
		),
		testutil.OptAfter(
			func(ctx context.Context) error {
...
func NewTest(t *testing.T) (*assert.Assertions, *postgres.Model, func()) {
	its := assert.New(t)
	tx, err := testutil.DefaultDB().Begin()
	its.Nil(err)
	return its, postgres.New(testutil.DefaultDB(), db.OptTx(tx)), func() {
		_ = tx.Rollback()
	}
}

// CreateTestQuiz creates a test quiz.
func CreateTestQuiz(its *assert.Assertions, mgr *postgres.Model) *types.Quiz {
